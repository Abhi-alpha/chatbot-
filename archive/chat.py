...
def build_prompt(question: str, snippets: List[Dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "Answer the user's question ONLY using the provided snippets.",
        "If the answer is not in the snippets, say you don't find it in this policy.",
        "Cite snippet numbers like [S1], [S2] when you use them.",
        "",
        "=== SNIPPETS ==="
    ]
...
class AskRequest(BaseModel):
    uin: str
    question: str
    top_k: Optional[int] = 5          # final snippets to use
    candidate_k: Optional[int] = 150   # how many to pull from DB before re-ranking
...
def ask(
    ...
    candidate_k = int(payload.candidate_k or 50)
    stmt = text("""
        SELECT
          c.id,
          c.section_id,
    ...
    # 4) Prepare candidates with similarity to the question
    candidates: List[Dict[str, Any]] = []
    for r in rows:
        # emb = list(r.embedding) if isinstance(r.embedding, (list, tuple)) else r.embedding
        emb = emp_to_float(r)
